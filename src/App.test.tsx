...
describe("App", () => {
  ...
  it("records best seeds on bump with current seed and pack height", async () => {
    ...
    const autoChk = screen.getByLabelText(
      /Auto Bump Seed/i
    ) as HTMLInputElement;
    if (autoChk.checked) {
    ...
    await waitFor(() =>
      expect(screen.getByText(/Best Seeds/i)).toBeInTheDocument()
    );
    ...
    await waitFor(() =>
      expect(screen.getByText(/Pack Height: 102\.0?mm/i)).toBeInTheDocument()
    );
    fireEvent.click(screen.getByRole("button", { name: /bump seed/i }));
  ...
  it("keeps only 10 best seeds and replaces the worst when a better one arrives", async () => {
    ...
    for (let s = 1; s <= 10; s++) {
      ...
      await waitFor(() =>
        expect(
          screen.getByText(new RegExp(`Pack Height: ${100 + s}\\.0?mm`, "i"))
        ).toBeInTheDocument()
      );
      fireEvent.click(screen.getByRole("button", { name: /bump seed/i }));
    }
    ...
    const bodyRowsAfter = within(screen.getByRole("table"))
      .getAllByRole("row")
      .filter((row) => row.querySelectorAll("td").length > 0);
